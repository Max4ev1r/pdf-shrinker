#!/usr/bin/env python3
"""
PDF Shrinker v4 - 苹果风明亮 UI + 桌面输出文件夹
...
CARD_BG = "#FFFFFF"       # 纯白卡片
ACCENT = "#007AFF"         # 苹果蓝
ACCENT_HOVER = "#0056CC"   # 深蓝
TEXT = "#1D1D1F"          # 苹果黑文字
TEXT_DIM = "#86868B"      # 次要文字
BORDER = "#D2D2D7"        # 边框灰
SUCCESS = "#34C759"        # 苹果绿
WARN = "#FF9500"          # 苹果橙
SHADOW_COLOR = "#00000010" # 卡片阴影
...
class ModernApp:
    ...
    def _build_ui(self):
        ...
        # 主卡片（苹果白卡片风格）
        card = tk.Frame(main_bg, bg=CARD_BG, bd=1, relief="flat",
        ...
        # 拖拽区域
        self.drop_frame = tk.Frame(card, bg="#F0F0F5", bd=1, relief="flat",
                                   highlightbackground=BORDER, highlightthickness=1,
                                   width=440, height=130, cursor="hand2")
        self.drop_frame.pack(pady=(0, 12))
        self.drop_frame.pack_propagate(False)

        self.drop_icon = tk.Label(self.drop_frame, text="⬡", font=("SF Pro Display", 36),
                                  bg="#F0F0F5", fg=ACCENT)
        self.drop_icon.pack(pady=(18, 0))

        self.drop_label = tk.Label(self.drop_frame, text="将 PDF 文件拖拽到这里",
                                   font=("SF Pro Text", 14), bg="#F0F0F5", fg=TEXT)
        self.drop_label.pack(pady=(4, 0))

        self.drop_hint = tk.Label(self.drop_frame, text="或点击选择文件",
                                  font=("SF Pro Text", 11), bg="#F0F0F5", fg=TEXT_DIM)
        self.drop_hint.pack(pady=(2, 8))

        self.drop_frame.bind("<Button-1>", self._browse)
        self.drop_icon.bind("<Button-1>", self._browse)
        self.drop_label.bind("<Button-1>", self._browse)
        self.drop_hint.bind("<Button-1>", self._browse)
        self.drop_frame.bind("<Enter>", lambda e: self.drop_frame.config(bg="#E8E8EF"))
        self.drop_frame.bind("<Leave>", lambda e: self.drop_frame.config(bg="#F0F0F5"))
    ...
    def _set_file(self, path):
        ...
        self.drop_hint.config(text=f"{size_mb:.1f}MB · 点击更换文件")
