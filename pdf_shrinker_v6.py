#!/usr/bin/env python3
"""
PDF Shrinker v6 - 苹果风 UI + Ghostscript 压缩（稳定压到 5MB 以下）
依赖: Python 3, Ghostscript
"""

import os, sys, shutil, io, subprocess
...
class PDFShrinkerApp:
    ...
    def _build_ui(self):
        ...
        if self.gs_path:
            ...
            gs_label = tk.Label(header, text="⚠️ 未检测到 Ghostscript，将自动安装",
    ...
    def _start_compress(self):
        if not self.file_path:
            return
        if not self.gs_path:
            messagebox.showwarning("提示", "正在使用内置压缩（Ghostscript 未检测到）\n请通过「启动PDFShrinker.bat」启动以获得最佳效果")
    ...
    def _reduce_pdf(self, input_path, output_path, original_size):
        """
        Ghostscript 压缩，逐一尝试直到 < 5MB
        """
        if not self.gs_path:
            raise RuntimeError("Ghostscript 未安装")

        # 尝试从低到高三档质量
        for profile, dpi, quality in [
            ("screen", 72, "低"),
            ("ebook", 150, "中"),
            ("prepress", 300, "高"),
        ]:
            self.root.after(0, lambda q=quality: self.status_label.config(text=f"压缩中... ({q}质量)"))
